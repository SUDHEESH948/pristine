...
export default function Pricing() {
  ...
  const solarEstimate = useMemo(() => {
    ...
    const systemSizes = [2.24, 3.36, 5.04, 7.84, 10.08];

    const systemSize =
      systemSizes.find((size) => size >= calculatedSize) ||
      systemSizes[systemSizes.length - 1];

    const matchingSystem = MICRO_PRICING.find(
      (item) =>
        Number(item.dc) === Number(systemSize) &&
        item.structure === "Without Structure"
    );

    const estimatedCost =
      matchingSystem?.price || Math.round(systemSize * 65000);
  ...
  const currentData = allCategories[category] || [];
  ...
  const structures = useMemo(() => {
    const values = currentData.map((item) => item.structure);
  ...
  const filteredData = useMemo(() => {
    return currentData.filter((item) => {
      ...
      const matchesSearch =
        !query ||
        String(item.modules).includes(query) ||
        String(item.dc).includes(query) ||
  ...
  const activeCategoryMeta = CATEGORIES.find((x) => x.id === category);
  ...
  return (
    <main className="min-h-screen bg-[#EAF4FC] font-['Plus_Jakarta_Sans',system-ui,sans-serif] text-slate-900 antialiased selection:bg-sky-100 selection:text-sky-900">
      ...
      {/* HERO SECTION */}
      <section className="relative flex min-h-screen min-h-[720px] flex-col justify-between overflow-hidden bg-gradient-to-b from-[#050f1a] via-[#0B253A] to-[#0d2a40] px-6 pt-28 pb-12 sm:px-8 lg:px-12 lg:pt-32 lg:pb-16">
        ...
        <div className="relative mx-auto flex w-full max-w-7xl flex-1 items-center">
          <div className="grid w-full items-center gap-12 lg:grid-cols-[1fr_430px] lg:gap-16">
            ...
            >
              ...
              <p className="mt-6 max-w-lg text-base font-normal leading-relaxed text-slate-300 sm:text-lg">
                Compare residential, micro-inverter, hybrid battery,
                and commercial solar systems, with structured pricing
                and subsidy schedules laid out plainly, panel by panel.
              </p>
              ...
              <div className="relative overflow-hidden rounded-[28px] border border-white/15 bg-white/[0.075] p-5 shadow-2xl shadow-black/40 backdrop-blur-2xl sm:p-6">
                ...
                >
                  ...
                  <p className="mt-3 text-[10px] leading-relaxed text-slate-400">
                    Based on your estimated monthly electricity
                    consumption and available residential system sizes.
                  </p>
                </motion.div>
                ...
                <p className="mt-3 text-center text-[8px] leading-relaxed text-slate-500">
                  Indicative estimate only. Final system size, pricing,
                  subsidy and savings depend on actual consumption,
                  site conditions and applicable government guidelines.
                </p>
              </div>
            </motion.div>
          </div>
            ...
            {CATEGORIES.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`font-display group relative flex min-w-max items-center gap-2.5 rounded-xl px-5 py-2.5 text-xs font-semibold uppercase tracking-[0.05em] transition-all duration-200 ${
                    active
                ...
                >
                  ...
                  {active && (
                    <motion.div
                      layoutId="tabPill"
                      className="absolute inset-0 -z-10 rounded-xl bg-[#0B253A]"
                      transition={{
                        type: "spring",
                        stiffness: 380,
                        damping: 30,
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <div className="relative min-w-[210px]">
                  ...
                  <select
                    ...
                    className="w-full appearance-none rounded-xl border border-slate-200/80 bg-slate-50/70 py-2.5 pl-9 pr-9 text-xs font-semibold text-slate-700 outline-none transition focus:border-sky-400 focus:bg-white focus:ring-2 focus:ring-sky-100"
                  >
                    {structures.map((item) => (
                ...
                >
                  <div>
                    <div className="relative overflow-hidden border-b border-slate-100 bg-[#0B253A] p-5 text-white">
                      <div className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-sky-500/20 blur-3xl transition-all duration-500 group-hover:bg-sky-400/30" />
                      ...
                      <div className="relative flex items-start justify-between gap-3">
                        ...
                        <motion.div
                          ...
                          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-400/10 text-sky-400 ring-1 ring-sky-400/20"
                        >
                          {category === "hybrid" ? (
                  ...
                  <div className="border-t border-slate-100 bg-slate-50/70 p-4">
                    <button
                      ...
                      className="group/button relative flex w-full items-center justify-center gap-1.5 overflow-hidden rounded-full bg-[#0284c7] py-3 text-xs font-bold uppercase tracking-wider text-white shadow-lg shadow-sky-200 transition-all duration-300 hover:bg-[#0369a1] hover:shadow-xl hover:shadow-sky-200 active:scale-95"
                    >
      ...
      <section className="border-t border-slate-200/80 bg-white px-6 py-16 lg:px-12">
        <div className="mx-auto max-w-7xl">
          ...
          >
            ...
            <h2 className="font-display mt-1 text-2xl font-bold uppercase tracking-[0.01em] text-slate-900 sm:text-3xl">
              Engineered for 30-Year Performance
            </h2>
            <p className="mt-2 text-sm text-slate-500">
              Every system includes comprehensive compliance testing,
              tier-1 warranty protection, and standardized installation
              norms.
            </p>
          </motion.div>
          ...
          >
            {[
              ...
              {
                icon: <CheckCircle2 className="text-sky-500" />,
      ...
      <AnimatePresence>
        {selectedItem && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-slate-950/75 p-4 pt-20 pb-20 backdrop-blur-sm sm:pt-24"
          ...
          >
            <motion.div
              ...
              className="relative my-auto max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white shadow-2xl ring-1 ring-slate-900/10"
            >
              {/* Modal Header */}
              <div className="sticky top-0 z-10 flex items-start justify-between overflow-hidden border-b border-slate-800 bg-[#0B253A] p-6 text-white">
                ...
                <div className="relative">
                  ...
                  <h3 className="font-display mt-1 text-lg font-bold uppercase tracking-[0.01em]">
                    {selectedItem.product}
                  </h3>
                  <p className="mt-0.5 text-xs text-slate-300">
                    {selectedItem.modules} Tier-1 Modules ·{" "}
                    {selectedItem.dc} kWp Output
                  </p>
                </div>

                <button
                  onClick={() => setSelectedItem(null)}
                  className="relative rounded-full bg-white/10 p-1.5 text-slate-300 transition hover:bg-sky-400/20 hover:text-white"
              ...
              <div className="grid grid-cols-2 gap-px bg-slate-100 text-xs">
                ...
                <ModalItem
                  ...
                  colSpan={selectedItem.battery ? 1 : 2}
                />
              </div>
              ...
              <div className="p-6">
                <div className="rounded-2xl border border-slate-200/80 bg-slate-50 p-5">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Base MRP (Standard BOM)</span>
                ...
                {selectedItem.subsidy && (
                  <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
                    ...
                    <p className="mt-1 text-[11px] text-emerald-700/80">
                      Credited directly to applicant bank account
                      post-metering commissioning.
                    </p>
                  </div>
                )}
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="group flex w-full items-center justify-center gap-2 rounded-full bg-[#0284c7] py-3.5 text-xs font-bold uppercase tracking-wider text-white shadow-lg shadow-sky-200 transition hover:bg-[#0369a1] hover:shadow-xl active:scale-95"
                  >
...
function InfoCard({ icon, title, text }) {
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-slate-50/50 p-6 transition-all hover:-translate-y-1 hover:border-sky-300 hover:bg-white hover:shadow-xl hover:shadow-sky-100">
